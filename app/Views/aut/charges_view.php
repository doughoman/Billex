...
<div class="container-fluid">
        ...
        <div class="main-table-div">
            ...
            <div class="mainrow-div">
                <div class="row1 chargeclick active_row">
                    <div class="select_customer_div" style="display: <?= (isset($_SESSION['customer_id']) ? "none" : "") ?>"><p>Select Customer</p></div>
                </div>
                <?php
                if (isset($_SESSION['customer_id'])) {
                    ?>
                    <input type="hidden" value="<?= base64_encode(encrypt($_SESSION['customer_id'])); ?>" id="charge_customer_id">
                    <div class="row1 chargeclick active_row customer_details" style="display: <?= (isset($_SESSION['customer_id']) ? "flex" : "") ?>">
                        ...
                        <div class="table-col recordhide">
                            <p class="customer_contact_name"><?= $contact_name; ?></p>
    ...
    <div class="row mt-3">
        <div class="col-md-8 col-lg-8">
            <form id="customer_charge">
                <input type="hidden" name="form[customer_id]" value="<?= (isset($_SESSION['customer_id']) ? $_SESSION['customer_id'] : "") ?>" id="customerlsiting">
                <div class="form-group row">
                    <label class="col-sm-3 text-right mob-left">Customer</label>
                    <div class="col-sm-9">
                        <div class="custom-dropdown big type_select_option">
                            <select class="selectpicker" data-live-search="true"  data-width="<?= (isset($_COOKIE['compact']) && $_COOKIE['compact'] == 1 ? "50%" : "100%"); ?>" title="Choose one of the item" id="customer_listing">
                                <?php
                                foreach ($customer_data as $value) {
                                    if ($value['status'] == "inactive") {
                                        continue;
                                    }
                                    echo '<option data-id="' . $value["customer_id"] . '" ' . (isset($_SESSION['customer_id']) && decrypt(base64_decode($value['customer_id'])) == $_SESSION['customer_id'] ? "selected" : "") . '>' . $value['name'] . '</option>';
                                }
                                ?>
                            </select>
                        </div>
    ...
    ?>
    <?php if (isset($_REQUEST['bill']) && $_REQUEST['bill'] == 1) {
        ?>
        <div class="btn-submitdiv pl-3 back_customer_btn">
            <a href="<?= base_url() ?>aut/billcharges" class="btn btn-adduser" id="back_customer_list">Back</a>
        </div>
        <?php
    }
    ?>
